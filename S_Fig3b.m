...
t20meanROIrank = trimmean(ranks,20,2);
% put data in the form of regression
Y = t20meanROIrank(CFG.goodroi);
...
title(msg)
saveas(gcf,[CFG.rsltsDir, 'fig/mrank_r_corr', '.png'])

% -----------------------------------------------

%% --- Histogram ---
his = histogram(round(t20meanROIrank),'BinMethod', 'integers')
his = his.Values;
N = length(his);
...
saveas(gcf,[CFG.rsltsDir, 'fig/mrank_hist', '.png'])
...
%ft_sourceplot([],mri) % uncomment if you wan to see raw mri in ortho view
...
for iroi = 1:CFG.nroi
    ...
    interp.rank(roivoxs) = t20meanROIrank(iroi);       
    t20meanROIrank(iroi)
end
...
camlight('right')
title('Mean rank')
saveas(gcf,[CFG.rsltsDir, 'fig/mrank_both', '.png'])
...
camlight('right')
title('Mean rank')
saveas(gcf,[CFG.rsltsDir, 'fig/mrank_left_in', '.png'])

ft_sourceplot(cfg, interp)
view([-90 0])
camlight('right')
title('Mean rank')
saveas(gcf,[CFG.rsltsDir, 'fig/mrank_left_out', '.png'])
...
camlight('right')
title('Mean rank')
saveas(gcf,[CFG.rsltsDir, 'fig/mrank_right_out', '.png'])
