% S_Fig5
% Distribution of number of clusters (hist, surf, regr)
...
% Version 2 - number of clusters from GM-model (so clusters contributed by majority
% of participants)
load(CFG.finalname) 
nclustmaj = zeros(length(CFG.goodroi),1);
for iroi = CFG.goodroi
    nclustmaj(iroi) = gm{iroi}.NumComponents;
end
% put data in the form of regression
Y = nclustmaj(CFG.goodroi);
...
ylabel('Number of clusters / area')
set(gca,'FontSize',18)
msg = ['y = (', num2str(b(1)), ') + (' , num2str(b(2)), ') * x']


title(msg)
saveas(gcf,[CFG.rsltsDir, 'fig/ncl_r_corr', '.png'])


%% --- Histogram ---
his = histogram(Y,'BinMethod', 'integers', 'BinLimits', [1 CFG.clustnum(end)])
his = his.Values;
N = length(his);
...
saveas(gcf,[CFG.rsltsDir, 'fig/ncl_hist', '.png'])
...
for iroi = 1:CFG.nroi
    ...
    % Version 1 - optclust from Silhouette 
    %interp.rank(roivoxs) = nclustopt(iroi);       
    % Version 2 - number of clusters from GM-model (so clusters contributed by majority of participants)
    interp.rank(roivoxs) = nclustmaj(iroi);       
end
...
saveas(gcf,[CFG.rsltsDir, 'fig/ncl_both', '.png'])
...
camlight('right')
title('Optimal number of clusters')
saveas(gcf,[CFG.rsltsDir, 'fig/ncl_left_in', '.png'])

ft_sourceplot(cfg, interp)
view([-90 0])
camlight('right')
title('Optimal number of clusters')
saveas(gcf,[CFG.rsltsDir, 'fig/ncl_left_out', '.png'])
...
camlight('right')
title('Optimal number of clusters')
saveas(gcf,[CFG.rsltsDir, 'fig/ncl_right_out', '.png'])
